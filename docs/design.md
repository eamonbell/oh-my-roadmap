...
- Workers always run LSP diagnostics on every file they touch before yielding. A worker may additionally run its task's own verification commands against its owned files — but only when the wave has a single worker or the dispatch is a genuine rework (`rework_of`/`reworkOf` set); running a full test suite, a whole-project build, or anything touching unowned files is never permitted. Workers record the exact commands they ran (a "Commands run:" section and/or "VERIFIED:" lines) in their note as receipts for the reviewer.
- An agent records a verification baseline once at the start of implementation via `omr_record_verification_baseline` (the pre-existing failing tests/counts before any wave runs). Reviewers judge each wave against that baseline — no new failures and no lost passes — rather than an absolute full-suite-green bar; pre-existing baseline failures are informational.
...
- Reviewers report structured findings with a severity (`pass`, `advisory`, `blocking_worker_fixable`, `blocking_needs_user`) via `omr_record_wave_review`; a plain string `findings` list is still accepted for back-compat. `pass`/`advisory` findings are dropped, `blocking_needs_user` findings open a canonical blocker, and `blocking_worker_fixable` findings become a `rework_queue` item instead of a blocker — a concrete fix a worker can make without a user decision. Sub-agent blocking notes route the same way: an optional `blockingKind` of `needs_user` (or unlabeled, for back-compat) mints a canonical blocker, while `worker_fixable` is recorded as an advisory note with no blocker.
- Blocking review findings and canonical blockers stop later waves until resolved or explicitly deferred; a deferred blocker can later be resolved once the deferred issue is actually fixed. A blocker whose id matches the `rework_of` marker on an active worker run is exempt from the write gate so the worker can perform that fix.
...
- The hub-first rework/re-review loop is bounded: `orchestration.max_review_cycles` (project config, default `2`) caps how many times the same wave can fail review before rework is routed back to a worker. Once a wave's failed-review count reaches the cap, the extension stops minting further `rework_queue` items and instead auto-mints (or reuses) a single canonical `blocking_needs_user` blocker whose description carries the accumulated findings history across all cycles for that wave, so the user sees the full trail in one place rather than resolving cycle by cycle.
...
`omr_task_briefing` is a separate, worker-facing read tool: a worker calls it once at the start of a task with its `owned_paths` and any `dependency_paths` to get file sizes, truncated head excerpts, and a one-hop import graph in one call, with `response_format: concise` or `detailed`. It replaces ad hoc exploratory reads for initial orientation; symbol outlines and diagnostics remain out of scope for it and come from the worker's own `xd://lsp`.

`omr_transition`'s tool description enumerates each operation's phase precondition (the roadmap/milestone phase, or active-milestone/change-request state, required before the operation is legal) alongside a canonical `write xd://omr_transition {...}` invocation example, so weaker models can self-correct illegal-phase calls without a failed round-trip.
