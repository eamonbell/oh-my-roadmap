...
function expectFindingsReportInstruction(content: string, commandName: string): void {
  ...
  expect(content).toContain("omr_submit_findings_report exactly once");
...
describe("roadmap commands", () => {
  test("omr:rm-new queues an agent command prompt with user arguments", async () => {
    const { commands, sentMessages, customMessages } = createHarness();
    const command = commands.get("omr:rm-new");
    ...
    expect(content).toContain("You are operating the oh-my-roadmap OMP extension command /omr:rm-new.");
    expect(content).toContain("User arguments:\nAdd billing workflows");
    expect(content).toContain("No active roadmap. Run /omr:rm-new to start a gated roadmap workflow.");
    ...
    expectFindingsReportInstruction(content, "omr:rm-new");
  });

  test("omr:rm-reopen sends command-specific reopen instructions", async () => {
    const { commands, sentMessages, customMessages } = createHarness();
    const command = commands.get("omr:rm-reopen");
    expect(command).toBeDefined();
    ...
    expect(sent.options).toBeUndefined();
    const content = sent.content;
    expect(content).toContain("You are operating the oh-my-roadmap OMP extension command /omr:rm-reopen.");
    expect(content).toContain("operation reopen_roadmap");
    expect(content).toContain("required reopen reason");
    expect(content).toContain("omr_update_roadmap");
    expect(content).toContain("omr_validate");
    ...
    expectFindingsReportInstruction(content, "omr:rm-reopen");
  });

  test("omr:rm-repair sends command-specific repair instructions", async () => {
    const { commands, sentMessages, customMessages } = createHarness();
    const command = commands.get("omr:rm-repair");
    expect(command).toBeDefined();
    ...
    expect(sent.options).toBeUndefined();
    const content = sent.content;
    expect(content).toContain("You are operating the oh-my-roadmap OMP extension command /omr:rm-repair.");
    expect(content).toContain("omr_validate");
    ...
    expect(content).toContain("omr_repair_roadmap");
    expect(content).toContain("Do not call reopen_roadmap");
    expectFindingsReportInstruction(content, "omr:rm-repair");
  });

  test("omr:ms-plan prompts for detailed test coverage decisions", async () => {
    const { commands, sentMessages, customMessages } = createHarness();
    const command = commands.get("omr:ms-plan");
    expect(command).toBeDefined();
  ...
  test("omr:ms-implement keeps user questions in the orchestrator role", async () => {
    const { commands, sentMessages, customMessages } = createHarness();
    const command = commands.get("omr:ms-implement");
    expect(command).toBeDefined();
    ...
    expect(content).toContain("omr_prepare_wave_dispatch");
    expect(content).toContain("omr_record_worker_dispatch");
    expect(content).toContain("omr_record_worker_transport_failed");
    expect(content).toContain("omr_record_worker_abandoned");
    expect(content).toContain("omr_record_wave_result");
    expect(content).toContain("omr_prepare_wave_review");
    expect(content).toContain("omr_record_wave_review");
    ...
    expect(content).toContain("Never call omr_record_wave_result with failed or blocked for a transport error");
    ...
    expect(content).toContain("Submit omr_submit_findings_report exactly once at the terminal point");
  ...
  test("blocker commands queue prompt-backed routing instructions", async () => {
    ...
    for (const name of ["omr:blk-list", "omr:blk-status", "omr:blk-resolve", "omr:blk-defer"]) {
      expect(commands.get(name)).toBeDefined();
    }

    await commands.get("omr:blk-list")?.handler("", testContext(cwd));
    await commands.get("omr:blk-status")?.handler("", testContext(cwd));
    await commands.get("omr:blk-resolve")?.handler(
      "blk_123 Fixed by reverting unowned edits",
      testContext(cwd),
    );
    await commands.get("omr:blk-defer")?.handler(
    ...
    for (const [prompt, name] of [
      [listPrompt, "omr:blk-list"],
      [statusPrompt, "omr:blk-status"],
    ] as const) {
      expect(prompt).toContain("omr_list_blockers");
      expect(prompt).toContain("/omr:blk-resolve <id> <resolution>");
      expect(prompt).toContain("/omr:blk-defer <id> <reason>");
      expect(prompt).toContain("/omr:rm-resume");
      expectFindingsReportInstruction(prompt, name);
    }

    expect(resolvePrompt).toContain("Parse user arguments as <blocker-id> <resolution>");
    expect(resolvePrompt).toContain("omr_resolve_blocker");
    expect(resolvePrompt).toContain("omr_validate");
    expect(resolvePrompt).toContain("tell the user to run /omr:rm-resume");
    expect(resolvePrompt).toContain("blk_123 Fixed by reverting unowned edits");
    expectFindingsReportInstruction(resolvePrompt, "omr:blk-resolve");

    expect(deferPrompt).toContain("Parse user arguments as <blocker-id> <reason>");
    expect(deferPrompt).toContain("omr_defer_blocker");
    expect(deferPrompt).toContain("omr_validate");
    expect(deferPrompt).toContain("tell the user to run /omr:rm-resume");
    expect(deferPrompt).toContain("blk_456 Accepted follow-up risk");
    expectFindingsReportInstruction(deferPrompt, "omr:blk-defer");
  });
  ...
  test("all prompt-backed commands queue agent command prompts while idle", async () => {
    ...
    for (const [index, name] of PROMPT_COMMANDS.entries()) {
      ...
      expect(content).toContain(`You are operating the oh-my-roadmap OMP extension command /${name}.`);
      expect(content).toContain("User arguments:\ntest args");
  ...
  test("prompt-backed commands queue follow-ups while busy", async () => {
    ...
    await commands.get("omr:rm-status")?.handler("", testContext(await testCwd(), false));

    expect(sentMessages).toHaveLength(1);
    expect(customMessages).toHaveLength(0);
    expect(sentMessages[0]?.options).toEqual({ deliverAs: "followUp" });
    expect(sentMessages[0]?.content).toContain("You are operating the oh-my-roadmap OMP extension command /omr:rm-status.");
  ...
  test("omr:fnd-clear clears the active findings report tile without prompting the model", async () => {
    ...
    const command = commands.get("omr:fnd-clear");
    expect(command).toBeDefined();
    await command?.handler("", testContext(await testCwd(), true, { setWidget }));

    expect(commands.has("omr:fnd-clear")).toBe(true);
  ...
  test("omr:fnd-clear reports unavailable UI without clearing a widget", async () => {
    const { commands, sentMessages, customMessages } = createHarness();
    const setWidget = () => {
  ...
  test("omr:rm-details renders local details without prompting the model", async () => {
    const { commands, sentMessages, customMessages } = createHarness();
    ...
    const command = commands.get("omr:rm-details");
    expect(command).toBeDefined();
    ...
    expect(commands.has("omr:rm-details")).toBe(true);
  ...
  test("omr:rm-details prompt controls queue through command prompt delivery", async () => {
    const { commands, sentMessages, customMessages } = createHarness();
    ...
    try {
      ...
      await commands.get("omr:rm-details")?.handler(
        "",
        {
          ...testContext(cwd),
          ui: {
      ...
      expect(sentMessages[0]?.content).toContain("omr_resolve_blocker");
      expect(sentMessages[0]?.content).toContain("omr_defer_blocker");
      expect(sentMessages[0]?.content).not.toContain("omr_submit_findings_report");
      expect(closeCount).toBe(1);
    } finally {
