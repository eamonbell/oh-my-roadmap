...
async function nextActionPlanImpl(cwd: string): Promise<NextActionPlan> {
	...
	if (!state.active || !state.roadmap) {
		return plan({
			...
			description: 'Create a roadmap with /omr:rm-new.',
			status: 'needs_input',
		...
		switch (state.changeRequest.status) {
			...
			case 'approved':
				return plan({
					...
					description: 'Start change implementation with /omr:ms-implement.',
					status: 'ready',
					safe_to_apply: true,
					scope: scopeFromState(state),
	...
	switch (state.roadmap.phase) {
		...
		case 'roadmap_approved':
			return plan({
				...
				description: 'Plan the next milestone with /omr:ms-plan.',
				status: 'ready',
				safe_to_apply: true,
				scope: scopeFromState(state),
		...
		case 'milestone_approved':
			return plan({
				...
				description: 'Start implementation with /omr:ms-implement.',
				status: 'ready',
				safe_to_apply: true,
				scope: scopeFromState(state),
				tool: transitionTool({operation: 'start_implementation'}),
		...
		case 'complete':
			if (hasPlannableMilestone(state)) {
				return plan({
					...
					description: 'Start the next planned milestone with /omr:ms-plan or create a post-implementation change request.',
					status: 'ready',
					safe_to_apply: true,
					scope: scopeFromState(state),
					tool: transitionTool({operation: 'start_milestone_planning'}),
