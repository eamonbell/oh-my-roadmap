...
import {milestoneNotesPath, roadmapDocPath} from './paths'
...
async function validateRoadmapOutline(cwd: string, roadmap: RoadmapState, errors: ValidationIssue[]): Promise<void> {
	if (!roadmap.roadmap_finalized) {
		errors.push(issue('roadmap.finalized.missing', 'Roadmap must be finalized with omr_update_roadmap before approval'))
	}
...
export async function validateRoadmapState(cwd: string): Promise<ValidationResult> {
	return await withDiagnosticTiming({
		component: 'core',
		operation: 'validation.validateRoadmapState',
		cwd,
		slowMs: 250,
	}, async () => {
		const errors: ValidationIssue[] = []
...
export async function validateImplementationGate(cwd: string): Promise<ValidationResult> {
	return await withDiagnosticTiming({
		component: 'core',
		operation: 'validation.validateImplementationGate',
		cwd,
		slowMs: 250,
	}, async () => {
