...
export function nextCommandForAction(next: NextActionPlan, state: LoadedState): string {
	const id = next.id
	if (id === 'roadmap:create') return '/omr:rm-new <goal>'
	if (id.includes(':blockers:open') || next.blockers.length > 0) return '/omr:blk-list'
	if (id.includes('milestone-check')) return '/omr:rm-new'
	if (id.includes('wave-flow-check')) return state.changeRequest ? '/omr:chg-request' : '/omr:ms-plan'
	if (id.startsWith('change:') && id.includes(':approve')) return '/omr:chg-request'
	if (id.startsWith('milestone:') && id.includes(':approve')) return '/omr:ms-plan'
	if (id.includes(':record-discovery') || id.startsWith('roadmap:') && id.includes(':approve')) return '/omr:rm-new'
	if (id.includes(':start-milestone-planning') || id.includes(':start-next-milestone-planning')) return '/omr:ms-plan'
	if (id.includes(':start-implementation') || id.startsWith('progress:')) return '/omr:ms-implement'
	if (id.includes(':record-closeout') || id.includes(':start-closeout')) return '/omr:ms-close'
	if (id.startsWith('change:') && (id.includes(':review') || id.includes(':closed'))) return '/omr:chg-close'
	if (id.includes(':complete')) return '/omr:chg-request <requested change>'
	if (id.includes(':validation:')) return '/omr:rm-status'
	return '/omr:rm-resume'
}
