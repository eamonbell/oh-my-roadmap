...
function activeLevel(): ConfiguredLevel {
	const raw = process.env.OH_MY_ROADMAP_LOG_LEVEL?.trim().toLowerCase()
...
function logDir(): string {
	return path.join(configuredHomeDir ?? process.env.HOME ?? os.homedir(), '.oh-my-roadmap', 'logs')
}
...
export async function logDiagnostic(options: DiagnosticLogOptions): Promise<void> {
	...
	try {
		...
		fallbackLogger?.warn('oh-my-roadmap diagnostic logging failed', {
